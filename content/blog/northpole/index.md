...
More on this later. It is too foggy to explain at this point.
...
NorthPole is a spatial architecture, in contrast to GPUs and TPUs that are temporal architectures [[Sze et al.](https://ieeexplore.ieee.org/abstract/document/8114708)]: this means that instead of having a single on-chip memory that all the processing engines (PEs) share, (part of) memory is co-located with the processing units. 
...
Takehome message: each core of TrueNorth is interconnected to the other using three networks: one for the activations (*i.e.*, the intermediate values between one layer and the other)
...
I don't see where my GPU is not behaving just like NorthPole :)
