...
Hence, IBM decide to try to emulate the brain with **TrueNorth**, a **4096 cores** chip packing **1 million neurons** and **256 thousand synapses**. Let's dive into its design!
...
* the CMOS process employed is a **low-power** one, with the goal of minimising **static power**. The technology node is **28nm**.
...
{{<
    figure
    src="/images/blog/dive-in-truenorth/imc.jpg"
...
The authors claim that **clock-gating** is not enough to save power when the circuit is not working, because of the complex control circuitry required for its implementation. For this reason, an asynchronous design is adopted to interconnect the cores, so that: 
* when no events are transmitted, the circuits are not working and no power is wasted distributing the clock.
* we get rid of the clock skew which would be present in such a large chip (4096 cores, remember!).

Moreover, the core control circuitry is asynchronous, so that the core operates only whene there is data to work on. Hence, communication among neurons in the same core is handled asynchronously. In particular, the clock is generated **locally** when there are somecomputations to be performed.
